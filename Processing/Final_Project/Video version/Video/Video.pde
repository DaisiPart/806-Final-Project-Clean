...
// Center rectangle geometry (use constants so video stays aligned)
final float RECT_X = 260;
final float RECT_Y = 180;
final float RECT_W = 675;
final float RECT_H = 425;
...
void setup() {
  ...
  player = minim.loadFile("Imagine.mp4");
  player.play(); 
  
  // NEW: load and loop a video file placed in the sketch's "data" folder
  // Replace "center.mp4" with your actual filename (MP4/MOV recommended)
  movie = new Movie(this, "Imagine.mp4");
  movie.loop();
...
// Song info
  ...
  // Center rectangle
  // Draw video first, then overlay a border so content looks framed
  if (movie != null) {
    // Scale video to the rectangle size
    image(movie, RECT_X, RECT_Y, RECT_W, RECT_H);
  }
  noFill();
  stroke(255);
  strokeWeight(2);
...
// Button clicks (control both audio and video)
  if (mouseX > 450 && mouseX < 525 && mouseY > btnY && mouseY < btnY + btnH) {
    ...
    player.pause();
    player.rewind();
    if (movie != null) { movie.pause(); movie.jump(0); }
  } else if (mouseX > 690 && mouseX < 765 && mouseY > btnY && mouseY < btnY + btnH) {
    player.rewind();
    player.play();
    if (movie != null) { movie.jump(0); movie.play(); }
  }
}

// Processing video callback: read frames as they arrive
